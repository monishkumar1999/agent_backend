...
typicallyRoute.use(cors(corsOptions));
typicallyRoute.options('*', cors(corsOptions));

// API to add a new Role
typicallyRoute.use("/add", checkJwt, async (req, res) => {
    const { name } = req.body;
    ...
    try {
        ...
        const existingRole = await Typeciallywork.findOne({ name, action: '0' });
        ...
        const roleInstance = new Typeciallywork({ name });
...
typicallyRoute.put("/edit", checkJwt, async (req, res) => {
    const { name, newName } = req.body;

    try {
        // Check if the new role name already exists
        const existingRole = await Typeciallywork.findOne({ name: newName, action: "0" });
        ...
        const roleInstance = await Typeciallywork.findOne({ name });
...
typicallyRoute.delete("/delete", checkJwt, async (req, res) => {
    const { _id } = req.body;
    ...
    try {
        ...
        const roleInstance = await Typeciallywork.findOne({ _id });
